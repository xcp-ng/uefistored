...
LAB_00404ac0:
  ...
  if (getopt_ret == -1) {
    ...
    if (*local_228 == '\0') {
      ...
      if (val == 0) {
        fprintf(stderr,"%s: ","varserviced_initialize");
        ...
        if (-1 < __fd) {
          ...
          printf("%s: ","varserviced_initialize");
          printf("%d vCPU(s)\n",(ulong)vcpu_count);
          fflush(stdout);
          while( true ) {
...
LAB_0040547e:
          ...
          if ((-1 < counter) && (cVar6 = load_auth_files(), cVar6 != '\0')) {
            ...
            if (argc == 0) {
              ...
              if ((counter == -1) ||
                 (counter = asprintf(stringbuf,"/local/domain/%u/varserviced-pid",(ulong)domain),
                 counter == -1)) {
                free(stringbuf[0]);
                free(local_210);
              }
...
switchD_00404b38_caseD_a:
                    /* WARNING: Subroutine does not return */
        __assert_fail("0","varserviced.c",0x313,"main");
      }
      goto LAB_00404ac0;
    default:
