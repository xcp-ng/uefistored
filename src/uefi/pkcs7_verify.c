...
**/
bool Pkcs7GetCertificatesList (
  const uint8_t  *P7Data,
  uint64_t        P7Length,
 uint8_t        **SignerChainCerts,
 uint64_t        *ChainLength,
 uint8_t        **UnchainCerts,
 uint64_t        *UnchainLength
  )
{
  bool          Status;
  uint8_t            *NewP7Data;
  ...
  *UnchainLength    = 0;
  ...
  if (CtxChain != NULL) {
    ...
    for (Index = 0; ; Index++) {
      ...
      WriteUnaligned32 ((uint32_t *) (CertBuf + OldSize), (uint32_t) CertSize);
  ...
  )
{
  ...
  //
  Status = (bool) PKCS7_verify (Pkcs7, NULL, CertStore, DataBio, NULL, PKCS7_BINARY);
